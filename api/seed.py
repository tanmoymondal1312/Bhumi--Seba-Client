#!/usr/bin/env python3
# Run: python3 api/seed.py
import sqlite3, os

db_file = os.path.join(os.path.dirname(__file__), 'bhumi.db')
...
CREATE INDEX idx_bkash_date ON bkash_records(date);
...
CREATE TABLE services_metadata (
...
# bcrypt hashes (generated via bcryptjs): PIN 9999 and PIN 1234
HASH_9999 = '$2a$10$3//kPWNPf5TgrKxPUdcRSuVSYn2TR5Ec9wIxD9oam4XNramO58LNS'
HASH_1234 = '$2a$10$vUhQ66RRq5sbXXk/IOvw5OQH8zjz9HRQnwmMcDYPTzhzUx9KiAHda'
...
db.execute("""INSERT INTO settings VALUES
    (1,1,1,'সাফল্য নিয়ে কোনো শর্টকাট নেই, সত্যতা ও সঠিক গ্রাহক সেবাই ব্যবসার আসল মূলধন।',
    5000,6000,1850,800,8000,NULL,5000,NULL,NULL)""")

# ── Services ──────────────────────────────────────────────────────────────────
db.executemany("INSERT INTO services_metadata VALUES (?,?,?,?,?,1,?)", [
...
# ── bKash records ─────────────────────────────────────────────────────────────
bkash = [
    ('bk-001','2026-05-10','14:05','IN',   2000,  None, 'মোঃ রনি',      'ইনকাম এন্ট্রি লিংক: জমি ক্রয়-বিক্রয় দলিল',        'BHUMABHUM1'),
    ('bk-002','2026-05-16','15:35','IN',    500,  None, 'মোঃ রনি',      'ইনকাম এন্ট্রি লিংক: ভূমি উন্নয়ন কর আবেদন',         'BHUMABHUM2'),
    ('bk-003','2026-05-20','10:00','OUT',  3000,  18.0, 'মোঃ রনি',      'সরকারি ফি পরিশোধ - নামজারি',                         'TRX20526001'),
    ('bk-004','2026-05-26','14:35','IN',   2500,  None, 'মোঃ রনি',      'ইনকাম এন্ট্রি লিংক: বাণিজ্যিক সম্পত্তি দলিল',       'BHUMABHUM3'),
    ('bk-005','2026-06-05','14:05','IN',    200,  None, 'করিম উদ্দিন',  'ইনকাম এন্ট্রি লিংক: মোঃ জাহাঙ্গীর - পর্চা',         'BHUMABHUM4'),
    ('bk-006','2026-06-09','13:35','IN',   3000,  None, 'মোঃ রনি',      'ইনকাম এন্ট্রি লিংক: বড় সম্পত্তি দলিল লিখন',         'BHUMABHUM5'),
    ('bk-007','2026-06-15','10:00','OUT',  5000,  30.0, 'মোঃ রনি',      'ব্যক্তিগত উত্তোলন',                                   'TRX20626001'),
    ('bk-008','2026-06-19','09:50','IN',    300,  None, 'মোঃ রনি',      'ইনকাম এন্ট্রি লিংক: সিরাজুল ইসলাম - নামজারি',       'BHUMABHUM6'),
    ('bk-009','2026-06-27','14:05','IN',   2000,  None, 'মোঃ রনি',      'ইনকাম এন্ট্রি লিংক: দলিল যাচাই ও লিখন',              'BHUMABHUM7'),
    ('bk-010','2026-07-01','11:05','IN',    200,  None, 'করিম উদ্দিন',  'ইনকাম এন্ট্রি লিংক: রাবেয়া বেগম - খতিয়ান',         'BHUMABHUM8'),
    ('bk-011','2026-06-25','09:00','PAYMENT',1850, None,'মোঃ রনি',      'বিদ্যুৎ বিল অনলাইন পেমেন্ট',                          'TRX20626002'),
]
db.executemany(
...
# ── Reminders ─────────────────────────────────────────────────────────────────
db.executemany("INSERT INTO reminders (id,title,date,is_completed) VALUES (?,?,?,?)", [
...
# ── Memos ─────────────────────────────────────────────────────────────────────
db.executemany("INSERT INTO memos (id,title,description,amount,image,entered_by,date,time) VALUES (?,?,?,?,?,?,?,?)", [
    ('memo-001','করিম মিয়ার নামজারি কেস',
     'দাগ নং ১২৩৪, খতিয়ান নং ৫৬৭, সদর উপজেলা। আবেদন জমা হয়েছে ০৪/০৫/২০২৬। অনুমোদনের জন্য অপেক্ষায়।',
     300,'','মোঃ রনি','2026-05-04','10:20'),
    ('memo-002','বাণিজ্যিক সম্পত্তি দলিল - রহিম ট্রেডার্স',
     'দলিল নং ৭৮৯/২০২৬। মৌজা: চাঁদপুর। দলিল সম্পন্ন, রেজিস্ট্রি বাকি। ক্লায়েন্ট ফোন: 01711-123456।',
     2500,'','মোঃ রনি','2026-05-26','14:35'),
    ('memo-003','জুন মাসের সারসংক্ষেপ',
     'মোট আয়: ৯,৯০০ টাকা। মোট ব্যয়: ১৭,৪৭০ টাকা। বিকাশ লেনদেন: ৫টি। নতুন কেস: ১৫টি। সম্পন্ন কেস: ১২টি।',
     0,'','মোঃ রনি','2026-06-30','17:00'),
    ('memo-004','সিরাজুল ইসলামের ভূমি কেস',
     '৩ একর জমির নামজারি ও পর্চা। দাগ: ৪৫৬, ৪৫৭। মৌজা: বাগানবাড়ি। বিভিন্ন উত্তরাধিকারীর মধ্যে বিরোধ আছে।',
     300,'','মোঃ রনি','2026-06-19','09:50'),
])

db.commit()
db.close()

print(f"✓ bhumi.db তৈরি হয়েছে ({os.path.getsize(db_file):,} bytes)")
print(f"  লগইন → মালিক: PIN 9999 | করিম উদ্দিন (স্টাফ): PIN 1234")
