...
export default function Dashboard({
  ...
  return (
    <div id="dashboard-tab" className="space-y-6">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl relative overflow-hidden">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              ...
              <div className="lg:col-span-5 grid grid-cols-1 gap-4">
                ...
                <div
                  id="card-income-before-today"
                  onClick={() => setShowIncomeBreakdown(true)}
                  title="ক্লিক করলে তারিখ অনুযায়ী আয়ের বিস্তারিত তালিকা দেখা যাবে"
                  className="bg-slate-950 border border-slate-850 hover:border-indigo-500/40 p-5 rounded-2xl flex flex-col justify-between relative overflow-hidden group cursor-pointer transition"
                >
                ...
                <div
                  id="card-income-up-to-today"
                  onClick={() => setShowIncomeBreakdown(true)}
                  title="ক্লিক করলে তারিখ অনুযায়ী আয়ের বিস্তারিত তালিকা দেখা যাবে"
                  className="bg-gradient-to-br from-indigo-950/25 to-slate-900/40 border border-indigo-505/10 hover:border-indigo-500/40 p-5 rounded-2xl flex flex-col justify-between relative overflow-hidden group cursor-pointer transition"
                >
                  <div className="flex items-center justify-between">
                    <div>
                  ...
                  <div className="mt-3 text-[10px] text-slate-400 z-10">
                    * আজকের সচল আয় ও ব্যয় সহ ১ তারিখ হতে এই পর্যন্ত মোট প্রকৃত আয় (আয় - ব্যয়) হিসাব। <span className="text-indigo-400 font-bold">ক্লিক করে দিনভিত্তিক তালিকা দেখুন →</span>
                  </div>
                </div>
              ...
              <div className="lg:col-span-7 bg-slate-950/50 border border-slate-850 rounded-2xl p-5">
                <span className="text-slate-400 text-xs font-semibold block mb-4.5">মাসিক খরচের খাতসমূহ (Fixed Deductions Detail)</span>
          ...
          {onAddReminder && (
            <form
              ...
              onSubmit={(e) => {
                ...
                onAddReminder(newTaskTitle.trim(), todayStr);
                setNewTaskTitle('');
              }}
            >
              <input
                ...
                placeholder="পেন্ডিং কাজ লিখুন... (যেমন: করিমের নামজারি ফলো-আপ)"
                className="flex-1 bg-slate-900 border border-slate-700 rounded-xl py-2 px-3 text-white text-xs focus:outline-none focus:border-indigo-500"
              />
              <button
                id="btn-quick-add-task"
                type="submit"
                className="px-3.5 py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold rounded-xl cursor-pointer transition flex items-center gap-1 shrink-0"
              >
              ...
              reminders.map(rem => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {onDeleteReminder && (
                      <button
                        id={`btn-delete-task-${rem.id}`}
                        onClick={(e) => {
                          e.stopPropagation();
                          onDeleteReminder(rem.id);
                        }}
                        title="কাজটি তালিকা থেকে মুছে ফেলুন"
                        className="p-1 rounded-lg text-slate-500 hover:text-red-400 hover:bg-slate-900 cursor-pointer transition"
                      >
      ...
      {/* DAILY INCOME BREAKDOWN MODAL — কোন তারিখে কত আয় */}
      {showIncomeBreakdown && (
        <div id="income-breakdown-modal-overlay" className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div
            className="absolute inset-0 bg-slate-950/85 backdrop-blur-sm"
            onClick={() => setShowIncomeBreakdown(false)}
          ></div>

          <div className="relative bg-slate-900 border border-slate-800 rounded-3xl p-6 max-w-lg w-full mx-auto shadow-2xl animate-in zoom-in duration-200 max-h-[85vh] flex flex-col">
            <div className="flex justify-between items-center mb-4 pb-3 border-b border-slate-800 shrink-0">
              <div>
                <h3 className="text-sm font-black text-white flex items-center gap-2">
                  ...
                  দিনভিত্তিক প্রকৃত আয়ের তালিকা ({currentMonthBgLabel})
                </h3>
                <p className="text-[10px] text-slate-500 mt-0.5">১ তারিখ থেকে আজ পর্যন্ত কোন তারিখে কত টাকা প্রকৃত আয় হয়েছে</p>
              </div>
              <button
                onClick={() => setShowIncomeBreakdown(false)}
              ...
              <table className="w-full text-xs">
                <thead className="sticky top-0 bg-slate-900">
                  <tr className="text-slate-500 border-b border-slate-800">
                    <th className="text-left py-2 px-2 font-semibold">তারিখ</th>
                    <th className="text-right py-2 px-2 font-semibold">আয় (ক্যাশ)</th>
                    <th className="text-right py-2 px-2 font-semibold">ব্যয়</th>
                    <th className="text-right py-2 px-2 font-semibold">প্রকৃত আয়</th>
                  </tr>
                </thead>
                <tbody>
                  {dailyIncomeBreakdown.map(row => (
                    <tr
                      key={row.dateStr}
                      className={`border-b border-slate-850/60 ${row.hasEntries ? '' : 'opacity-40'}`}
                    >
                      <td className="py-2 px-2">
                        <span className="font-bold text-slate-200">{toBanglaDigits(row.day)} তারিখ</span>
                        <span className="text-[9px] text-slate-500 font-mono block">{row.dateStr}</span>
                      </td>
                      {row.hasEntries ? (
                        <>
                          <td className="py-2 px-2 text-right font-mono text-emerald-400">৳{row.income.toLocaleString('bn-BD')}</td>
                          <td className="py-2 px-2 text-right font-mono text-rose-400">৳{row.expense.toLocaleString('bn-BD')}</td>
                          <td className={`py-2 px-2 text-right font-mono font-bold ${row.profit >= 0 ? 'text-indigo-300' : 'text-rose-400'}`}>৳{row.profit.toLocaleString('bn-BD')}</td>
                        </>
                      ) : (
                        <td colSpan={3} className="py-2 px-2 text-right text-slate-600 italic">কোনো এন্ট্রি নেই (দোকান বন্ধ)</td>
                      )}
                    </tr>
                  ))}
                </tbody>
                <tfoot>
                  <tr className="border-t-2 border-indigo-500/30">
                    <td className="py-2.5 px-2 font-black text-indigo-300">সর্বমোট</td>
                    <td className="py-2.5 px-2 text-right font-mono font-bold text-emerald-400">
                      ৳{dailyIncomeBreakdown.reduce((s, r) => s + r.income, 0).toLocaleString('bn-BD')}
                    </td>
                    <td className="py-2.5 px-2 text-right font-mono font-bold text-rose-400">
                      ৳{dailyIncomeBreakdown.reduce((s, r) => s + r.expense, 0).toLocaleString('bn-BD')}
                    </td>
                    <td className="py-2.5 px-2 text-right font-mono font-black text-indigo-300">
                      ৳{dailyIncomeBreakdown.reduce((s, r) => s + r.profit, 0).toLocaleString('bn-BD')}
                    </td>
                  </tr>
                </tfoot>
              </table>
